                    ...
                    <div className="flex flex-col gap-20">
                        ...
                        <div className="grid lg:grid-cols-3 gap-16 lg:gap-10">
                            {dummyProducts.map((product, index) => (
                                <div key={index} className="w-full h-full flex flex-col justify-between border-2 border-grey-300 rounded-2xs hover:shadow-outer-card hover:border-stc-400 hover:scale-98 transition-all duration-300 cursor-pointer">
                                      <div className="relative bg-stc-300 py-20 px-20 w-full h-full rounded-tl-2xs rounded-tr-2xs">
                                        {product.tag && (
                                          <p className={clsx("z-50 absolute bg-stc-200 py-8 px-18 max-w-fit rounded uppercase text-grey-800 font-button", product.tagPosition === "left" ? "left-20" : "right-20")}>
                                            {product.tag}
                                          </p>
                                        )}
                                      ...
                                      <div className="bg-grey-500 p-20 flex flex-col gap-20 w-full uppercase rounded-bl-2xs rounded-br-2xs">
                                        ...
                                        <div className="flex justify-between items-center">
                                            ...
                                            <button onClick={() => addToCart(product)} className="p-10 btn bg-stc-800 font-button text-stc-200 border-[0.6px]" disabled>
                                                Add to Bag
                                            </button>
                                        </div>
