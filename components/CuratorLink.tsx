...
export default function CuratorLink({
    content
}: { content: Content}) {
    const { link, type, tag, title, description } = content;

    return (
        <Link href={link} className="w-full h-full flex flex-col justify-between border-2 border-grey-300 rounded-2xs hover:shadow-outer-card hover:border-stc-400 hover:scale-98 transition-all duration-300 cursor-pointer">
            ...
            <div className="bg-grey-400 p-20 flex flex-col gap-8 w-full rounded-bl-2xs rounded-br-2xs">
                <p className="text-stc-200 font-caption uppercase">{type}&nbsp;{tag && <span>· {tag}</span>}</p>
