export function ContactCard({ children }: { children: React.ReactNode }) {
  return <div className="border-[0.5px] border-grey-400 bg-grey-400 py-31 px-16 lg:px-34">{children}</div>;
}

export function ContactCardTitle({ children }: { children: React.ReactNode }) {
  return <p className="font-title uppercase text-grey-200">{children}</p>;
}

export function ContactCardItems({ children }: { children: React.ReactNode }) {
  return <div className="divide-y-[0.6px] divide-grey-300">{children}</div>;
}
    ...
    <p className="py-24 font-title font-satoshi text-grey-250">
      ...
      <span>
        ...
        <a
          ...
          className="text-stc-200 hover:text-stc-200/70 transition-colors duration-150"
        >
          {handle}
        </a>
      </span>
